<!DOCTYPE html>
<html lang="es">

<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>Historial de Mapas (Admin)</title>

  <link rel="stylesheet" href="/css/generales.css">
  <link rel="stylesheet" href="/css/header.css">
  <link rel="stylesheet" href="/css/mapa.css">

  <link rel="stylesheet" href="https://unpkg.com/leaflet@1.9.4/dist/leaflet.css">
  <script src="https://unpkg.com/leaflet@1.9.4/dist/leaflet.js"></script>
  
  <script src="/js/lib/leaflet.heat.js"></script>
</head>

<body>
  <header class="header" data-breakpoint="1050">
    <div class="logo">
      <img src="/images/logo.png" alt="logo">
    </div>

    <button class="menu-toggle" aria-controls="site-nav" aria-expanded="false">
      <svg class="menu-toggle__icon" viewBox="0 0 24 24">
        <path d="M3 6h18M3 12h18M3 18h18" stroke="currentColor" stroke-width="2" stroke-linecap="round"></path>
      </svg>
    </button>

    <nav class="nav" id="site-nav">
      <a href="/index.html">Inicio</a>
      <a href="/pages/mapas.html">Mapas</a>
      <a href="/pages/sobre-nosotros.html">Sobre Nosotros</a>
      <a href="/pages/FAQ.html">Faq</a>
      <a href="/pages/contacto.html">Contacto</a>
      <a href="/pages/login.html" class="login">Inicia Sesión</a>
      <a href="/pages/registro.html" class="login">Registrarse</a>
    </nav>
  </header>

  <div class="map-container">
    <aside class="sidebar">
      <h2>Filtros</h2>

      <div class="filter-section">
        <h3>Capas</h3>
        <label><input type="checkbox" id="layer-bikes" checked>Bicicletas</label>
        <label><input type="checkbox" id="layer-pollution" checked>Contaminación</label>
      </div>

      <div class="filter-section">
        <h3>Tipo de gas</h3>
        <div class="select-wrapper">
          <select id="gas-select">
            <option value="general">Calidad General</option>
            <option value="pm25">PM 2.5</option>
            <option value="pm10">PM 10</option>
            <option value="no2">NO2</option>
            <option value="o3">O3</option>
          </select>
        </div>
      </div>

      <div class="filter-section">
        <h3>Fecha</h3>
        <input type="date" id="date-filter" class="date-input">
      </div>

      <div class="filter-section">
        <h3>Hora</h3>
        <input type="range" id="hour-slider" min="0" max="23" value="12" step="1">
        <span id="hour-display">12:00</span>
      </div>
    </aside>

    <button id="toggle-sidebar">
      <svg viewBox="0 0 24 24">
        <path d="M3 6h18M3 12h18M3 18h18" stroke="currentColor" stroke-width="2" stroke-linecap="round" />
      </svg>
    </button>

    <div id="map"></div>

    <div class="map-legend">
      <div class="legend-item">
        <span class="dot good"></span>
        <span>Buena</span>
      </div>
      <div class="legend-item">
        <span class="dot moderate"></span>
        <span>Moderada</span>
      </div>
      <div class="legend-item">
        <span class="dot unhealthy"></span>
        <span>Mala</span>
      </div>
    </div>
  </div>

  <section class="data-table">
    <h2>Medidas Mostradas en el Mapa</h2>
    <div class="table-container">
      <table id="measures-table">
        <thead>
          <tr>
            <th>Hora</th>
            <th>Latitud</th>
            <th>Longitud</th>
            <th>Tipo Contaminante</th>
            <th>Valor</th>
            <th>Color</th>
          </tr>
        </thead>
        <tbody></tbody>
      </table>
    </div>
  </section>

  <footer class="ft">
    <div class="ft__wrap ft--two-cols">
      <div class="ft__brand">
        <span class="ft__logo">
          <img src="/images/logo_footer.png" alt="logo">
        </span>
        <address class="ft__addr">
          Calle Juanba Peris, Nº 11, Gandia, España<br>
          <a href="tel:+34345464356">(434) 546-4356</a><br>
          <a href="mailto:contact@eolos.com">contact@eolos.com</a>
        </address>
      </div>

      <div class="ft__links">
        <nav class="ft__col">
          <a href="/index.html">Inicio</a>
          <a href="/pages/mapas.html">Mapas</a>
          <a href="/pages/sobre-nosotros.html">Sobre nosotros</a>
        </nav>

        <nav class="ft__col">
          <a href="/pages/contacto.html">Contacto</a>
          <a href="/pages/login.html">Iniciar sesión</a>
        </nav>

        <nav class="ft__col">
          <a href="#">Facebook</a>
          <a href="#">Twitter</a>
          <a href="#">Instagram</a>
        </nav>
      </div>
    </div>

    <div class="ft__copy">
      Copyright © eolos.
    </div>
  </footer>

  <script src="/js/scripts/mapa.js" type="module"></script>
</body>

</html>
